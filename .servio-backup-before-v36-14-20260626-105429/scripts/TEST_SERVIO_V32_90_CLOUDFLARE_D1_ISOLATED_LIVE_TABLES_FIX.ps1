...
Write-Host "Testing SERVIO v32.90 Cloudflare Day-Ahead D1 isolated live tables fix..." -ForegroundColor Cyan
$today = Get-Date
...
Assert ([string]$health.version -like '*v32.90*') ("Wrong deployed version: " + $health.version)
...
if(-not $SkipSync){
  ...
  Assert ([string]$opcom.guard -like '*v32.90*' -or [string]$opcom.mode -like '*v3290*') "OPCOM v32.90 guard marker missing"
  ...
  if(-not $opcom.ok -or [int]($opcom.parsedRecords ?? 0) -eq 0){
    Write-Warning "OPCOM sync did not parse records, but route returned guarded JSON instead of Cloudflare 1101. Run DIAGNOSE script to inspect days[]."
  }
...
Write-Host "v32.90 Cloudflare Day-Ahead D1 isolated live tables fix test passed." -ForegroundColor Green
