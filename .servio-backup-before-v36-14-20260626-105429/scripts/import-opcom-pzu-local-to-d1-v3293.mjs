...
const VERSION = 'v32.93-local-opcom-d1-import-fallback';
...
async function main(){
  ...
  const run={id:runId,reason:'v32.93-local-opcom-csv-import-fallback',inserted:all.length,updated:0,skipped:0,totalRaw:all.length,preparedRecords:all.length,finishedAtUtc:now,days};
  const syncSql=`INSERT OR REPLACE INTO servio_live_sync_runs (id, reason, inserted, updated, skipped, total_raw, finished_at_utc, raw_json) VALUES (${sqlString(runId)}, 'v32.93-local-opcom-csv-import-fallback', ${all.length}, 0, 0, ${all.length}, ${sqlString(now)}, ${sqlString(JSON.stringify(run))});\n`;
  const sql=schema+'BEGIN TRANSACTION;\n'+body+'\n'+syncSql+'COMMIT;\n';
  const sqlPath=path.join(outDir,`servio-v3293-opcom-import-${from}_to_${to}.sql`);
  const reportPath=path.join(outDir,`servio-v3293-opcom-import-${from}_to_${to}.json`);
