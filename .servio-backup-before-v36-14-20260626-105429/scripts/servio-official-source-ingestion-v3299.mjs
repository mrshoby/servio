...
async function fetchEntsoeDate(date, token, eurRon){
  ...
  for(const v of entsoeVariants(date)){
    ...
    try{ const xml=await fetchText(url.toString(),{headers:{accept:'application/xml,text/xml,*/*','user-agent':'SERVIO-v32.99-official-ingestion'}},30000); const records=parseEntsoeXml(xml,eurRon).filter(r=>r.date===date); attempts.push({name:v.name,ok:true,records:records.length,complete:records.length>=96,periodStart:v.periodStart,periodEnd:v.periodEnd,bytes:xml.length}); if(records.length>best.length){best=records; bestName=v.name;} if(records.length>=96) break; }
...
async function probeTranselectrica(){
  ...
  const out=[]; for(const url of urls){ try{ const text=await fetchText(url,{headers:{'User-Agent':'SERVIO-v32.99-source-health'}},20000); out.push({url,ok:true,bytes:text.length,hasBalancing:/echilibrare|balancing|dezechilibru/i.test(text)}); }catch(e){ out.push({url,ok:false,error:String(e.message||e)}); } }
  return out;
}
function mdReport(report){ return `# SERVIO v32.99 Hybrid Official Source Relay Report\n\n- Generated: ${report.finishedAtUtc}\n- Range: ${report.from} → ${report.to}\n- SERVIO URL: ${report.servioUrl}\n- Relay mode: ${report.relayMode}
...
| Final SERVIO effective source | - | ${report.summary.finalCompleteDays} | ${report.summary.finalFailedDays} |\n\n## Notes\n\n- ENTSO-E uses token from ENTSOE_API_TOKEN.\n- OPCOM is fetched by GitHub runner when possible, and by Windows local scheduled relay when GitHub/Cloudflare are blocked, then pushed to SERVIO API.\n- SERVIO API writes to Cloudflare D1 via /api/servio/ingest/*.\n`; }
async function main(){
  ...
  const relayMode=arg('relay-mode', process.env.SERVIO_RELAY_MODE || (process.env.GITHUB_ACTIONS==='true'?'github-actions':'windows-local-manual'));
  const strict=hasFlag('strict') || String(process.env.SERVIO_STRICT_COMPLETE||'').toLowerCase()==='true';
  const report={version:VERSION,relayMode,servioUrl,from,to,source,eurRon,dryRun,strict,startedAtUtc:new Date().toISOString(),days:[],pushes:[],transelectrica:[],finalHealth:[],summary:{entsoeRecords:0,entsoeCompleteDays:0,entsoeFailedDays:0,opcomRecords:0,opcomCompleteDays:0,opcomFailedDays:0,finalCompleteDays:0,finalFailedDays:0}};
  const days=dateRange(from,to,10);
  for(const date of days){
    const day={date};
    if((source==='all'||source==='entsoe') && token){ const r=await fetchEntsoeDate(date,token,eurRon); day.entsoe={records:r.records.length,complete:r.complete,bestAttempt:r.bestAttempt,attempts:r.attempts}; report.summary.entsoeRecords+=r.records.length; if(r.complete) report.summary.entsoeCompleteDays++; else report.summary.entsoeFailedDays++; if(r.records.length && !dryRun){ const push=await pushRecords(servioUrl,secret,'/api/servio/ingest/entsoe',{sourceMode:'official-live',source:'ENTSO-E Transparency Platform',reason:'hybrid-relay-entsoe-v3299',records:r.records,expectedIntervals:96,pushedBy:'servio-hybrid-relay-v3299'}); report.pushes.push({date,source:'entsoe',prepared:r.records.length,push}); } }
  ...
  if(!dryRun){
    for(const date of days){
      try{
        const health=await fetchText(`${servioUrl.replace(/\/$/,'')}/api/servio/live/source-health?date=${date}`,{headers:{accept:'application/json'}},12000).then(t=>JSON.parse(t));
        report.finalHealth.push({date, ok:Boolean(health.ok), records:health.final?.records||0, complete:Boolean(health.final?.complete), selectedSourceMode:health.final?.selectedSourceMode||health.final?.sourceMode||null, sourceHealth:health.sourceHealth||[]});
        if(health.final?.complete) report.summary.finalCompleteDays++; else report.summary.finalFailedDays++;
      }catch(e){ report.finalHealth.push({date,ok:false,error:String(e.message||e)}); report.summary.finalFailedDays++; }
    }
  }
  report.finishedAtUtc=new Date().toISOString();
  const stamp=`${from}_to_${to}`.replace(/[^0-9A-Za-z_\-]+/g,'_');
  const jsonPath=path.join(outDir,`servio-v3299-official-ingestion-${stamp}.json`);
  const mdPath=path.join(outDir,`servio-v3299-official-ingestion-${stamp}.md`);
  await fs.writeFile(jsonPath,JSON.stringify(report,null,2));
  await fs.writeFile(mdPath,mdReport(report));
  console.log(JSON.stringify({ok:true,version:VERSION,relayMode,from,to,source,summary:report.summary,finalHealth:report.finalHealth,report:jsonPath,markdown:mdPath,pushes:report.pushes.length},null,2));
  if(!dryRun && !report.pushes.length) process.exitCode=2;
  if(strict && report.summary.finalFailedDays>0) process.exitCode=3;
}
