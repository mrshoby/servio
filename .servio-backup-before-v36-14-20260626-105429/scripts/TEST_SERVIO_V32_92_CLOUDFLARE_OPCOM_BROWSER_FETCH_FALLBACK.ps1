...
Write-Host "Testing SERVIO v32.92 Cloudflare OPCOM browser fetch fallback..." -ForegroundColor Cyan
$today = Get-Date
...
Assert ([string]$health.version -like '*v32.92*') ("Wrong deployed version: " + $health.version)
...
if(-not $SkipSync){
  ...
  Assert ([string]$opcom.guard -like '*v32.92*' -or [string]$opcom.mode -like '*v3292*') "OPCOM v32.92 guard marker missing"
  ...
  if(-not $opcom.ok -or [int]($opcom.parsedRecords ?? 0) -eq 0){
    Write-Warning "OPCOM sync did not parse records, but route returned guarded JSON instead of Cloudflare 1101. Run DIAGNOSE v32.92 script to inspect days[]."
  }
...
Write-Host "v32.92 Cloudflare OPCOM browser fetch fallback test passed." -ForegroundColor Green
