...
const checks = [
  ["build version v4.36", worker.includes("servio-grid-map-v4.36-template-registry")],
  ...
  ["Electricity Maps live grid preserved", worker.includes('GRID_MAP_PROVIDER || "electricitymaps"') && worker.includes("/v4/electricity-mix/latest") && worker.includes("GRID_MAP_ZONES.length") && worker.includes("full-europe-single-signal-v4.36-template-registry")],
...
console.log("SERVIO v4.36 template registry guards OK.");
