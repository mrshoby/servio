...
const checks = [
  ["build version v4.34", worker.includes("servio-grid-map-v4.34-column-matrix-mapping-trainer")],
  ...
  ["Electricity Maps live grid preserved", worker.includes('GRID_MAP_PROVIDER || "electricitymaps"') && worker.includes("/v4/electricity-mix/latest") && worker.includes("GRID_MAP_ZONES.length") && worker.includes("full-europe-single-signal-v4.34-column-matrix-mapping-trainer")],
...
console.log("SERVIO v4.34 column/matrix mapping trainer guards OK.");
