...
const checks = [
  ...
  ["energy lab no technical dump", !app.includes('Analize încărcate') && !app.includes('Caută fișier, vendor, layout') && !app.includes('Upload curbe energetice și vezi rezultatele') && !app.includes('Operator workspace')],
  ["energy lab parser keeps full csv raw", app.includes('return { ...info, raw: `SHEET: Text/CSV\\n${raw}` };') && app.includes('buildEnergyLabAnalysis') && app.includes('parseEnergyLabTimestamp')],
