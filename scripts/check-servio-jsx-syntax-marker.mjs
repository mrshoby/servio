...
const checks = [
  ["build version v4.38", worker.includes("servio-grid-map-v4.38-granularity-normalization")],
  ...
  ["granularity normalization UI exists", app.includes("dlcnormalization") && app.includes("Granularity Normalization") && app.includes("60m→15m") && app.includes("15m→60m")],
  ...
  ["Electricity Maps live grid preserved", worker.includes('GRID_MAP_PROVIDER || "electricitymaps"') && worker.includes("/v4/electricity-mix/latest") && worker.includes("GRID_MAP_ZONES.length") && worker.includes("full-europe-single-signal-v4.38-granularity-normalization")],
...
console.log("SERVIO v4.38 granularity normalization guards OK.");
