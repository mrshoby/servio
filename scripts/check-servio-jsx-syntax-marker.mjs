...
const checks = [
  ["build version v4.40", worker.includes("servio-grid-map-v4.40-consumption-dataset-analyzer")],
  ...
  ["Electricity Maps live grid preserved", worker.includes('GRID_MAP_PROVIDER || "electricitymaps"') && worker.includes("/v4/electricity-mix/latest") && worker.includes("GRID_MAP_ZONES.length") && worker.includes("full-europe-single-signal-v4.40-consumption-dataset-analyzer") || worker.includes("full-europe-single-signal-v4.39-data-quality-validation")],
...
console.log("SERVIO v4.40 consumption dataset analyzer guards OK.");
