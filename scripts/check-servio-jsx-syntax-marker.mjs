...
const checks = [
  ["build version v4.41", worker.includes("servio-grid-map-v4.41-production-dataset-analyzer")],
  ...
  ["storage migrated to v4.41", app.includes("servio.dataLearning.v441") && app.includes("servio.dataLearning.v440")],
  ...
  ["Electricity Maps live grid preserved", worker.includes('GRID_MAP_PROVIDER || "electricitymaps"') && worker.includes("/v4/electricity-mix/latest") && worker.includes("GRID_MAP_ZONES.length") && worker.includes("full-europe-single-signal-v4.41-production-dataset-analyzer")],
...
console.log("SERVIO v4.41 production dataset analyzer guards OK.");
