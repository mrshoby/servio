...
$NodeScript = Join-Path $Root "scripts\import-opcom-pzu-local-to-d1-v3294.mjs"
...
Write-Host "SERVIO v32.94 local OPCOM import + PowerShell Wrangler execute fix" -ForegroundColor Cyan
...
$SqlPath = Join-Path $Out "servio-v3293-opcom-import-$($From)_to_$($To).sql"
$ReportPath = Join-Path $Out "servio-v3293-opcom-import-$($From)_to_$($To).json"
...
Write-Host "  .\scripts\TEST_SERVIO_V32_94_LOCAL_OPCOM_IMPORT_RESULT.ps1 -Url `$Url" -ForegroundColor Gray
