...
await browser.close();
const summary={ ok:results.every(r=>r.ok && !r.whiteScreen && !r.horizontalOverflow && !(r.iframeCount>0) && !(r.forbiddenVisible||[]).length) && results.filter(r=>r.screenshot).length>0 && results.reduce((a,r)=>a+(r.consoleErrors?.length||0),0)===0, version:'v36.9', stage, baseUrl, routes:routes.length, screenshots:results.filter(r=>r.screenshot).length, failed:results.filter(r=>!r.ok).length, whiteScreens:results.filter(r=>r.whiteScreen).length, overflow:results.filter(r=>r.horizontalOverflow).length, consoleErrors:results.reduce((a,r)=>a+(r.consoleErrors?.length||0),0), iframes:results.reduce((a,r)=>a+(r.iframeCount||0),0), forbiddenVisible:[...new Set(results.flatMap(r=>r.forbiddenVisible||[]))], generatedAtUtc:new Date().toISOString() };
