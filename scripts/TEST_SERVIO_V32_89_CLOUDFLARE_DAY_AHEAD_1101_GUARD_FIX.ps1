...
Write-Host "Testing SERVIO v32.89 Cloudflare Day-Ahead 1101 guard fix..." -ForegroundColor Cyan
$today = Get-Date
...
Assert ([string]$health.version -like '*v32.89*') ("Wrong deployed version: " + $health.version)
...
if(-not $SkipSync){
  ...
  Assert ([string]$opcom.guard -like '*v32.89*' -or [string]$opcom.mode -like '*v3289*') "OPCOM v32.89 guard marker missing"
...
Write-Host "v32.89 Cloudflare Day-Ahead 1101 guard fix test passed." -ForegroundColor Green
